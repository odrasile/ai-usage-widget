...
Implementar y mantener un widget de escritorio para Windows que monitoriza el uso de herramientas AI coding mediante sus CLIs locales.
...
- Integracion local con Codex y Claude Code.
- Build funcional en Windows.
...
1. Detectar instalacion con `where.exe codex`.
...
1. Detectar con `where.exe claude`.
2. Ejecutar `claude`.
...
- Solo ejecutar comandos en whitelist: `where.exe`, `codex`, `claude`.
